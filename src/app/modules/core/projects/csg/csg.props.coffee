define (require)->
  ...
  class CSG.Properties
    ...
    @addFrom = (result, otherproperties) ->
      for propertyname of otherproperties
        continue  if propertyname is "_transform"
        continue  if propertyname is "_merge"
        if (propertyname of result) and (typeof (result[propertyname]) is "object") and (result[propertyname] instanceof CSG.Properties) and (typeof (otherproperties[propertyname]) is "object") and (otherproperties[propertyname] instanceof CSG.Properties)
  ...
  class CSG.Connector
    ...
    axisLine: ->
      new CSG.Line3D(@point, @axisvector)
    
    extend: (distance) ->
      # creates a new Connector, with the connection point moved in the direction of the axisvector
      newpoint = @point.plus(@axisvector.unit().times(distance))
      new CSG.Connector(newpoint, @axisvector, @normalvector)
